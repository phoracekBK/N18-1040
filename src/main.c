	...
	{	
		...
		{	
			...
			if(util_copy_file(c_string_get_char_array(this->q_hotfolder_main_path), c_string_get_char_array(this->pci_hotfolder_in_path), job->camera_name) == 0)
			{
				...
				offset = core_get_next_csv_line(this->bkcore_csv_content, this->bkcore_csv_pos, bkcore_csv_line, 512);					
				if(offset > 0)
					this->info->total_stemps_number = atoi(bkcore_csv_line);
				...
				{					
					if(offset > 0)
						this->info->total_sheet_number = atoi(bkcore_csv_line);
				}
				else
				{
					if((offset <= 0) || (this->info->total_sheet_number != atoi(bkcore_csv_line)))
						core_set_machine_error(this, MACHINE_ERR_CSV_SHEET_NUM);
				}
			}
			else
...
uint8_t core_print_cancel(core * this)
{
	...
	if(c_string_len(this->printed_job_name) > 0)
	{
		...
		while(this->machine_state != MACHINE_STATE_WAIT && ttl < 100)
		{
...
gboolean gui_cyclic_interupt(gpointer param)
{
	...
	char buffer[512];
	sprintf(buffer, "GIS status: %s", c_string_get_char_array(this->core_ref->print_controller_status));
	gtk_label_set_text(GTK_LABEL(this->control_page->lbl[GC_LBL_GIS_STATUS]), buffer);

	sprintf(buffer, "Status: %s", core_get_error_str(this->core_ref));
	gtk_label_set_text(GTK_LABEL(this->control_page->lbl[GC_LBL_MACHINE_STATUS]), buffer);
...
gui_control_page * gui_control_page_new(gui_base * gui_base_ref)
{
	...
	gtk_widget_set_size_request(GTK_WIDGET(this->log_report_scroll_area), width/2, height/4);
	...
	gtk_fixed_put(GTK_FIXED(this->page), this->log_report_scroll_area, width/4, height/3*2);

	for(int i = 0; i< GC_LBL_N; i++)
	{
		this->lbl[i] = gtk_label_new(NULL);

		if(i <= GC_LBL_RJ_COUN_STATUS)
		{
			gtk_fixed_put(GTK_FIXED(this->page), this->lbl[i], 100, (5*height/24)+50+(40*i));
		}
		else
		{
			gtk_fixed_put(GTK_FIXED(this->page), this->lbl[i], width-200, (5*height/24)+50+(40*(i-GC_LBL_RJ_COUN_STATUS)));
		}
	}
	...
	for(i = 0; i < GC_BTN_N; i++)
	{
		...
		{
			gtk_fixed_put(GTK_FIXED(this->page), this->btn[i], width/4+(120*i), (11*height/24)+60);
		}
		else
		{
			gtk_fixed_put(GTK_FIXED(this->page), this->btn[i], 3*width/4-80, (11*height/24)+60);
		}
	}
