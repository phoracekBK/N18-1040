...
void read_bkcore_csv_info(q_job * job, char * address);
...
void core_safety_system_out(core * this)
{
	if(this->machine_state == MACHINE_STATE_WAIT)	
	{
		if(this->manual_freed > 0)
		{
			uint8_t pulse_state =  c_pulse_high(this->ena_pulse, 100);
			io_card_set_output(this->io_card_ref, IO_CARD_A1, A1_OUT_7_XBF, pulse_state);
					...
					if(job_name_len > 0)
					{
						...
						else
						{
							...
							else
							{
								...
								if(job->flag == 'k')
									read_bkcore_csv_info(job, c_string_get_char_array(this->q_hotfolder_main_path));
							}
						}
...
{
	/* open and read csv */
	char * bkcore_csv_content = util_load_csv(address, job->bkcore_name, NULL);
	...
	if(bkcore_csv_content != NULL)
	{
		...
		while((row < 2) && (bkcore_csv_content[pos] != 0))
		{
			...
			if(isdigit(bkcore_csv_content[pos]))
			{
				if(row == 0)
				{
					job->stamp_number = (job->stamp_number*10) + (bkcore_csv_content[pos]-48);
				}
				else if(row == 1)
				{
					job->sheet_number = (job->sheet_number * 10) + (bkcore_csv_content[pos]-48);
				}
			}
...
void gui_signals(gui * this)
{
	...
	g_signal_connect(G_OBJECT(this->control_page->ena_switch), "state-set", G_CALLBACK(gui_control_page_set_ena_callback), this->core_ref);
	g_signal_connect(G_OBJECT(this->control_page->xbf_pulse), "clicked", G_CALLBACK(gui_control_page_manual_sheet_feed), this->core_ref);
...
gui_control_page * gui_control_page_new(gui_base * gui_base_ref)
{
	...
	gtk_switch_set_active(GTK_SWITCH(this->ena_switch), io_card_get_output(this->gui_base_ref->core_ref->io_card_ref, IO_CARD_A1, A1_OUT_10_ENA) > 0);	
...
gboolean gui_control_info_box_draw_callback(GtkWidget * widget, cairo_t * cr, gpointer param)
{
	...
	cairo_text_extents(cr, core_machine_get_state_str(core_ref), &ext_machine_status);
	cairo_move_to(cr, width-(width/4-350)/2-ext_machine_status.width, 5*height/24+200);
	cairo_show_text(cr, core_machine_get_state_str(core_ref));
...
void gui_control_page_language(gui_control_page * this)
{
	...
	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].gui_job_state, JOB_STATE)));
	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].gui_job_name, JOB_NAME)));
	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].gui_job_order, JOB_ORDER)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].gui_job_sheet_number, JOB_SHEET_NUMBER)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].gui_job_stamp_number, JOB_STAMP_NUMBER)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].gui_job_date_time, JOB_DATE_TIME)));
	...
	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_report_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].rep_csv_state, REP_FINISH_STATE)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_report_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].rep_csv_name, REP_NAME)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_report_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].rep_csv_rejected_sheets, REP_REJECTED_SHEET_NUMBER)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_report_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].rep_csv_sheet_number, REP_SHEET_NUMBER)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_report_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].rep_csv_stamp_number, REP_STAMP_NUMBER)));

	gtk_tree_view_append_column (GTK_TREE_VIEW(this->job_report_list), 
					GTK_TREE_VIEW_COLUMN(gui_control_page_new_tree_column((gchar*) multi_lang[lang_index].rep_csv_date_time, REP_DATE_TIME)));
	...
	while(i < array_list_size(core_ref->job_list))
	{
		...
		else
		{
			job_status = multi_lang[core_ref->lang_index].print_state_wait;
				...
				JOB_ORDER, job->order,
				JOB_SHEET_NUMBER, job->sheet_number, 
				JOB_STAMP_NUMBER, job->stamp_number,
				JOB_DATE_TIME, job->date_time, -1);
		i++;
	}
...
void gui_control_page_update_report_csv_list(gui_control_page * this)
{
	...
	{
		char * report_csv_content = util_load_csv(core_get_job_report_hotfolder_path(this->gui_base_ref->core_ref), 
							c_string_get_char_array(array_list_get(this->report_csv_list, index)), NULL);
			...
			while((report_csv_content[pos] != 0) && (par < 6))
			{
				switch(par)
				{
					case 0:
						...
						{
               						gtk_list_store_set (this->job_report_list_store, &iter, REP_NAME, c_string_get_char_array(buffer), -1);
						}
					...
					case 1:
						if(report_csv_content[pos] == '\n')
               						gtk_list_store_set (this->job_report_list_store, &iter, REP_FINISH_STATE, c_string_get_char_array(buffer), -1);
						else
							c_string_add_char(buffer, report_csv_content[pos]);
						break;
					...
					case 2:
						if(report_csv_content[pos] == '\n')
						{
               						gtk_list_store_set (this->job_report_list_store, &iter, REP_REJECTED_SHEET_NUMBER, temp_int_val, -1);
						}
						else
						{
					...
					case 3:
						if(report_csv_content[pos] == '\n')
						{
               						gtk_list_store_set (this->job_report_list_store, &iter, REP_SHEET_NUMBER, temp_int_val, -1);
						}
						else
						{
							if(isdigit(report_csv_content[pos]))
					...
					case 4:
						if(report_csv_content[pos] == '\n')
						{
        						gtk_list_store_set (this->job_report_list_store, &iter, REP_STAMP_NUMBER, temp_int_val, -1);
						}
						else
						{
							if(isdigit(report_csv_content[pos]))
					...
					case 5:
						...
						if(report_csv_content[pos] == '\n')
						{
							gtk_list_store_set (this->job_report_list_store, &iter, REP_DATE_TIME, c_string_get_char_array(buffer), -1);
						}
...
void gui_control_page_open_report_csv (GtkTreeView *tree_view, gpointer param)
{
	...
	GtkTreeIter iter;
		...
		sprintf(cmd, "nohup $(cat %s/%s.csv | %s) > /dev/null 2>&1&", core_get_job_report_hotfolder_path(this->core_ref), csv_name, DEFAULT_TEXT_EDITOR);
		system(cmd);
                g_free (csv_name);
        }	
}
